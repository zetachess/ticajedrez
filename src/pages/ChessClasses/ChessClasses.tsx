...
export function ChessClasses() {
	...
	useEffect(() => {
		const example = document.querySelector(".tic-topic__code-example");
		const symbols = document.querySelector(".tic-topic__symbols");
		if (example && symbols) example.insertAdjacentElement("afterend", symbols);
	}, []);
	useEffect(() => {
		...
		<main>
			...
			{topics.map((topic) => {
				...
				return <section className="tic-topic" id={id} key={topic.number}>
					...
					<div className="tic-topic__content"><h2>{topic.title}</h2>{topic.number === "01.5" && <img className="tic-topic__resources-map" src={resourcesMap} alt="Recursos de ajedrez: radio, películas, plataformas, aplicaciones y vídeos" />}<div className="tic-topic__body">{topicLeads[topic.number] && <p className="tic-topic__lead"><strong>{topicLeads[topic.number]}</strong></p>}{topic.number === "01" && <img className="tic-topic__tic-map-image" src={ticChessMap} alt="Historia del ajedrez: del tablero clásico a la inteligencia artificial" />}{(topic.number === "01.5" ? [] : fullGuide[topic.number]).map((paragraph) => {
					...
					{topic.number === "02" && <details className="tic-topic__symbols"><summary><FileCode2 size={19} aria-hidden="true" /><span>Símbolos de anotación ajedrecística</span><ChevronDown size={18} aria-hidden="true" /></summary><div>{chessAnnotationSymbols.map(([symbol, meaning]) => <p key={`${symbol}-${meaning}`}><code>{symbol}</code><strong>{meaning}</strong></p>)}</div></details>}
